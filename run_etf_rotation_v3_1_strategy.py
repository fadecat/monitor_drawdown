...
import etf_rotation_v3_1_strategy as strategy
import inspect_etf_trend_sources as sources


DEFAULT_CONFIG_PATH = Path("etf_rotation_v3_1_config.yaml")
SOURCE_OUTPUT_ROOT = Path(".test_artifacts/etf_trend_sources")
DEFAULT_OUTPUT_ROOT = Path(".test_artifacts/etf_rotation_v3_1_strategy")


def load_rotation_config(path: Path | str = DEFAULT_CONFIG_PATH) -> dict[str, Any]:
    payload = yaml.safe_load(Path(path).read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("rotation v3.1 config must be a mapping")
    return payload
...
def collect_rotation_inputs(
    ...
    if unresolved_targets:
        _ = output_root
        resolved_results = [sources.resolve_target(target) for target in unresolved_targets]
...
def _trim_candidate_output(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "score_25_positive": candidate.get("score_25_positive"),
        "qualified": candidate.get("qualified"),
        "rejection_reason": candidate.get("rejection_reason"),
        "short_confirmation_threshold": candidate.get("short_confirmation_threshold"),
    }


def build_rotation_evaluation(
    snapshots: list[dict[str, Any]],
    ...
    for snapshot in snapshots:
        ...
        ordered_rows.append({"label": label})

    evaluated_candidates = strategy.apply_relative_median_short_confirmation(
        candidates=pending_candidates,
        strategy_config=strategy_config,
    )
    evaluated_by_label = {
        str(candidate.get("label") or "").strip(): candidate for candidate in evaluated_candidates
    }
    ...
    for row in ordered_rows:
        ...
        if evaluated_candidate is not None:
            candidate_metrics.append(_trim_candidate_output(evaluated_candidate))
        else:
            candidate_metrics.append(row)

    ranked_candidates = [
        _trim_candidate_output(item) for item in strategy.rank_candidates(evaluated_candidates)
    ]
    return candidate_metrics, ranked_candidates, evaluated_candidates
...
def build_portfolio_decision(
    ...
    return {
        ...
        "rejected_candidates": [
            _trim_candidate_output(item)
            for item in portfolio_decision.get("rejected_candidates") or []
            if isinstance(item, dict)
        ],
        "fallback_holding": (
            _trim_candidate_output(portfolio_decision["fallback_holding"])
            if isinstance(portfolio_decision.get("fallback_holding"), dict)
            else None
        )
    }
...
def build_defensive_holding(
    ...
    for snapshot in snapshots:
        ...
        return {
            "label": label,
            "latest_snapshot": {"selected_primary": selected_primary},
            "score_25": None,
            "annualized_return_25": None,
            "r_squared_25": None,
            "return_10d": None,
...
def run(
    ...
    candidate_metrics, ranked_candidates, evaluated_candidates = build_rotation_evaluation(
        snapshots=snapshots,
        strategy_config=strategy_config,

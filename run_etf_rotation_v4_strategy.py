...
DEFAULT_CONFIG_PATH = Path("etf_rotation_v4_config.yaml")
SOURCE_OUTPUT_ROOT = Path(".test_artifacts/etf_trend_sources")
DEFAULT_OUTPUT_ROOT = Path(".test_artifacts/etf_rotation_v4_strategy")


def load_rotation_config(path: Path | str = DEFAULT_CONFIG_PATH) -> dict[str, Any]:
    payload = yaml.safe_load(Path(path).read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("rotation v4 config must be a mapping")
    return payload
...
def validate_snapshot_coverage(config: dict[str, Any], snapshots: list[dict[str, Any]]) -> None:
...
def load_series_records(selected_primary: dict[str, Any], source_output_root: Path) -> list[dict[str, Any]]:
...
def _trim_candidate_output(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "short_confirmation_variant": candidate.get("short_confirmation_variant"),
        "volatility_20d": candidate.get("volatility_20d"),
        "volatility_adjusted_return_10d": candidate.get("volatility_adjusted_return_10d"),
        "self_relative_short_percentile": candidate.get("self_relative_short_percentile"),
        "short_confirmation_passed": candidate.get("short_confirmation_passed"),
        "absolute_floor": candidate.get("absolute_floor"),
...
def build_defensive_holding(snapshots: list[dict[str, Any]], defensive_labels: set[str]) -> dict[str, Any] | None:
    for snapshot in snapshots:
        label = str(snapshot.get("label") or "").strip()

...
import run_etf_rotation_v3_1_strategy as module
...
def test_run_rotation_v3_1_strategy_keeps_single_positive_score_asset_out_of_defensive_fallback(
    monkeypatch,
):
...
def test_run_rotation_v3_1_strategy_uses_defensive_fallback_when_absolute_floor_rejects_single_positive_score_asset(
    monkeypatch,
):
    root = _make_workspace_tmp("run_rotation_v3_1_absolute_floor_fallback")
    config_path = root / "rotation_v3_1.yaml"
    config_path.write_text(
        """
risk_targets:
...
""".strip(),
    ...
    assert [item["label"] for item in result["ranked_candidates"]] == []
    assert result["candidate_metrics"][0]["qualified"] is False
    assert result["candidate_metrics"][0]["rejection_reason"] == "return_10d_below_absolute_floor"
    assert result["candidate_metrics"][0]["short_confirmation_threshold"] == -0.03
    assert result["portfolio_decision"]["selection_reason"] == "fallback_defensive_asset"
    assert [item["label"] for item in result["portfolio_decision"]["selected_holdings"]] == ["CASH"]

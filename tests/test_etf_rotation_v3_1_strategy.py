...
import etf_rotation_v3_1_strategy as module
...
def test_build_rotation_candidate_v3_1_keeps_positive_score_without_applying_short_filter():
...
def test_apply_relative_median_short_confirmation_enforces_absolute_floor():
    candidates = [
        {
            "label": "A",
            "score_25": 1.5,
    ...
    qualified_by_label = {item["label"]: item["qualified"] for item in result}
    rejection_reason_by_label = {item["label"]: item["rejection_reason"] for item in result}
    threshold_by_label = {item["label"]: item["short_confirmation_threshold"] for item in result}

    assert qualified_by_label == {"A": False, "B": False, "C": True}
    assert rejection_reason_by_label["A"] == "return_10d_below_absolute_floor"
    assert rejection_reason_by_label["B"] == "return_10d_below_absolute_floor"
    assert rejection_reason_by_label["C"] == ""
    assert threshold_by_label == {"A": -0.03, "B": -0.03, "C": -0.03}
...
def test_apply_relative_median_short_confirmation_rejects_single_score_positive_asset_below_absolute_floor():
    result = module.apply_relative_median_short_confirmation(
        candidates=[
            {
                "label": "A",
    ...
    assert result[0]["rejection_reason"] == "return_10d_below_absolute_floor"
    assert result[0]["short_confirmation_threshold"] == -0.03


def test_calculate_weighted_trend_metrics_v3_1_matches_v2_math():

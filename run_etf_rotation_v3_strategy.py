...
import etf_rotation_v3_strategy as strategy
import inspect_etf_trend_sources as sources


DEFAULT_CONFIG_PATH = Path("etf_rotation_v3_config.yaml")
SOURCE_OUTPUT_ROOT = Path(".test_artifacts/etf_trend_sources")
DEFAULT_OUTPUT_ROOT = Path(".test_artifacts/etf_rotation_v3_strategy")


def load_rotation_config(path: Path | str = DEFAULT_CONFIG_PATH) -> dict[str, Any]:
    payload = yaml.safe_load(Path(path).read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("rotation v3 config must be a mapping")
    return payload

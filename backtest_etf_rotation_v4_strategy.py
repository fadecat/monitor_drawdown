...
import etf_rotation_v4_strategy as strategy
import run_etf_rotation_v4_strategy as runner


DEFAULT_OUTPUT_ROOT = Path(".test_artifacts/etf_rotation_v4_backtest")
...
def build_symbol_contributions(holding_periods: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "average_period_return": _clean_float(
                    float(item["_total_period_return"]) / periods_count if periods_count else 0.0
                ),
    ...
    return sorted(rows, key=lambda item: (-float(item["total_contribution"]), str(item["symbol"])))
...
def _build_invalid_candidate_metric_row(
    signal_date: str,
    label: str,
    ...
    return {
        ...
        "short_confirmation_variant": None,
        "volatility_20d": None,
...
def _build_candidate_metric_row(signal_date: str, candidate: dict[str, Any]) -> dict[str, Any]:
    row = runner._trim_candidate_output(candidate)
...
def _build_defensive_holding(
    ...
    for label in defensive_labels:
        ...
        return {
            ...
            "volatility_adjusted_return_10d": None,
            "self_relative_short_percentile": None,
            "short_confirmation_passed": True,
            "absolute_floor": None,
            "history_window": None,
        }
    return None
...
def replay_rotation_strategy(
    ...
    short_confirmation_history_days = int(strategy_config.get("short_confirmation_history_days") or 252)
    volatility_lookback_days = int(strategy_config.get("volatility_lookback_days") or 20)
    required_records = max(lookback_days, short_confirmation_history_days + 1, volatility_lookback_days + 1)
    ...
    for date_index in range(start_index, len(signal_dates) - 1):
        ...
        pending_rows_by_label: dict[str, dict[str, Any]] = {}
        ...
        for label, metadata in metadata_by_label.items():
            ...
            history_records = [
                record for record in series_records if str(record.get("date") or "").strip() <= signal_date
            ]

            if not history_records or str(history_records[-1].get("date") or "").strip() != signal_date:
                pending_rows_by_label[label] = _build_invalid_candidate_metric_row(
                    signal_date=signal_date,
                    label=label,
                    metadata=metadata,
            ...
            candidate = strategy.build_rotation_candidate_v4(
        ...
        evaluated_candidates = [item for item in pending_candidates if bool(item.get("qualified"))]
        evaluated_by_label = {
            str(candidate.get("label") or "").strip(): candidate for candidate in pending_candidates
        }

        for label, metadata in metadata_by_label.items():
            if label not in risk_labels:
                continue
            evaluated_candidate = evaluated_by_label.get(label)
            if evaluated_candidate is not None:
                daily_candidate_metrics.append(_build_candidate_metric_row(signal_date, evaluated_candidate))
                continue
            invalid_row = pending_rows_by_label.get(label)
            if invalid_row is not None:
                daily_candidate_metrics.append(invalid_row)

        ranked_candidates = strategy.rank_candidates(pending_candidates)
        ...
        selected_primary = (
            selected_snapshot.get("selected_primary", {}) if isinstance(selected_snapshot, dict) else {}
        )
...
def run_backtest(
    ...
    targets = risk_targets + defensive_targets
    metadata_by_label: dict[str, dict[str, Any]] = {}
    series_by_label: dict[str, list[dict[str, Any]]] = {}

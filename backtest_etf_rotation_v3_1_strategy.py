...
import run_etf_rotation_v3_1_strategy as runner


DEFAULT_OUTPUT_ROOT = Path(".test_artifacts/etf_rotation_v3_1_backtest")
...
def build_symbol_contributions(
    ...
    return sorted(
        rows,
        key=lambda item: (-float(item["total_contribution"]), str(item["symbol"])),
    )
...
def _build_candidate_metric_row(
    signal_date: str,
        ...
        for label, metadata in metadata_by_label.items():
            ...
            if evaluated_candidate is not None:
                daily_candidate_metrics.append(
                    _build_candidate_metric_row(signal_date, evaluated_candidate)
                )
                continue
        ...
        daily_positions.append(
            {
                ...
                "annualized_return_25": (
                    selected.get("annualized_return_25") if isinstance(selected, dict) else None
                ),
